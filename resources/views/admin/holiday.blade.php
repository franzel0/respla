...
<script>
   $( document ).ready(function() {
		...
		$('#company_id').on('change', function(e){
			var route = '{{ route("company.department.holiday.index", [":company", ":department"]) }}';
		...
		<div class="col-md-10 col-md-offset-1">
			<div class="panel panel-default">
				...
				<div class="panel-body">					
  					<div class="row">
						@if (isset($holiday))
                        {!! Form::model($holiday, ['method' => 'PATCH', 'route' => ['company.department.holiday.update', $company_id, $department_id, $holiday->id]]) !!}
                        @else
                        {!! Form::open(['route' => ['company.department.holiday.store', $company_id, $department_id]]) !!}
                        @endif
  							...
  							<div class="panel panel-default">
								...
								<div class="panel-body listitems">
    								...
    								<div class="input-group">
    									<a href="{{ route('company.department.holiday.create', [$company_id, $department_id])  }}" class="btn btn-success">Ferien hinzufügen</a>
    								</div>
									...
									@foreach ($holiday_list as $hol)
										<li data-id="{{ $hol->id }}">
											<a href="{{ route('company.department.holiday.edit', [$company_id, $department_id, $hol->id]) }}">{{ $hol->name }},  {{date("Y", strtotime($hol->date_from))}}</a>
										</li>
									@endforeach
									</ul>
									@endif	
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="input-group bgcolor">
                                                    <span class="input-group-addon">Von...</span>
                                                        @if (isset($holiday))
                                                        {!! Form::text('date_from', date("d.m.Y", strtotime($holiday->date_from)), ['class' => 'form-control from', 'required' => 'required']) !!}
                                                        @else
                                            ...
                                            <div class="col-md-6">
                                                <div class="input-group bgcolor">
                                                    <span class="input-group-addon">Von...</span>
                                                        @if (isset($holiday))
                                                        {!! Form::text('date_to', date("d.m.Y", strtotime($holiday->date_to)), ['class' => 'form-control to', 'required' => 'required']) !!}
                                                        @else
